		...
		return {
			...
			getWeather: function(TopCtrl) {				
				...
				if (this.checkCache() || mode === 'web') {
					...
					*	2，匹配出城市代码
					*   3，通过雅虎jsonp接口转接获取weather.com.cn的天气信息
					*
					*/
					$.getScript(cityInfoUrl, function() {						
						var cityCodeArr = cityStr.match(remote_ip_info.city + ':([0-9]+)');		
						weatherUrl = 'http://api.vas.wpscdn.cn/weather/v1/forecast/' + remote_ip_info.city + '/0/0';	 
						
						//请求成功回调函数
						var callback = function(a,b,c){
							if(c){
								var txt = MiniApi.JsDeCodeString(c);		//AES解码
								var obj = JSON.parse(txt);	
								if(obj.result == 'ok'){	
									self.setWeatherInfo(TopCtrl, obj.data, true);		
								}
							}
						}						
						//请求天气接口
						MiniApi.jsPost(weatherUrl, '', '3000', callback, '', '');		
					})
				} else {
					//读取缓存
					this.setWeatherInfo(TopCtrl, JSON3.parse(MiniApi.readRegSetting('weather', true)));
				}
			...
			/**
			*	设置天气信息
			*	@参数 weatherInfo: 天气信息json对象
			*		  cache: 是否放进缓存
			*/
			setWeatherInfo: function(TopCtrl, weatherInfo, cache) {
				
				try {									
					var currentCity = weatherInfo.current_city,					//当前城市
					    temperature = weatherInfo.weather_data.temperature,		//温度
					    weather = weatherInfo.weather_data.weather;				//天气
			...
			getCityCode: function() {
				var cityTime = MiniApi.readRegSetting('cityTime'),
					defer = $.Deferred();

				//判断是否在2星期内
				if (new Date() - cityTime < 1209600000) {
					cityCode = MiniApi.readRegSetting('cityCode', true);

					//判断从缓存获取的citycode是否正确的citycode;
					var matchArr = cityStr.match('([\u4e00-\u9fa5]+):' + cityCode + '[\u4e00-\u9fa5]*');
					if (matchArr) {						
						defer.resolve(cityCode, matchArr[1]);						
					} else {
						defer.reject();
					}
				} else {
					$.ajax({
						...
						success: function() {
							var cityName = remote_ip_info.city,
								cityCodeArr = cityStr.match(cityName + ':([0-9]+)');						
							if (cityCodeArr) {
								cityCode = cityCodeArr[1];
								defer.resolve(cityCode, cityName);
								MiniApi.writeRegSetting('cityCode', cityCode, true);								
								MiniApi.writeRegSetting('cityTime', +new Date());
							} else {
								defer.reject();
							}
						},
