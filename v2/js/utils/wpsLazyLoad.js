define(['jquery'], function($) {
	/**
	*	图片延迟加载插件
	*   @author liangweisheng 
	*
	*
	*/
	
	//图片延迟加载实现函数
	var WpsLazyLoad = function(window, $) {
		//初始化蚕食
		var defaultOption = {
				threshold: 0, //灵敏度
	            failure_limit : 0,
		...
		var optionHandel = {
			...
			setOption : function(element, opt) {
				return element.data("_WpsLazyLoadOption_", opt);
			},

			//获取参数对象
			getOption : function(element) {
				return element.data("_WpsLazyLoadOption_");
			},

			//删除对象参数
			removeOption : function(element) {
				return element.removeData("_WpsLazyLoadOption_");
		...
		*/		
		function add(element, opt) {
			...
			//初始化容器的存储数据
			if (!$container.data("_WpsLazyLoad_container_")) {
				$container.data("_WpsLazyLoad_container_", containerData);
			} else {
				containerData = $container.data("_WpsLazyLoad_container_");				
			}
			...
			if (!containerData.isBind || containerData.event !== opt.event) {
				$container.bind(opt.event, function(e){
					var data = $(this).data("_WpsLazyLoad_container_"),
						elementMap = data.elementMap;

					//循环判断元素是否满足显示要求
					$.each(elementMap, function(key, el) {
						if (el.data("_WpsLazyLoadOption_")) {
			...
			$.each(opt.event.split(" "), function(i, event) {
				if (event === 'scroll') {
					...
					$container.trigger(e, 'lazyLoad');
					return;
				}
				$container.trigger(event, 'lazyLoad');
			});
		...
		*/
		function remove(element) {
			...
			//删除对应容器中的映射关系
			delete $(opt.container).data("_WpsLazyLoad_container_").elementMap[opt._index];
			//删除元素中的_WpsLazyLoadOption_数据
		...
		//
		$.fn.WpsLazyLoad = function(opt) {
			return this.each(function() {
				switch($.type(opt)) {
					//不传入参数或者为json对象，则进行add操作
					case "undefined":
					case "object": 
					WpsLazyLoad.add($(this), opt);
					break;
					//传入参数为string类型，则判断为方法调用
					case "string":
					var args = Array.prototype.slice.call(arguments, 1);
					args.unshift($(this));
					WpsLazyLoad[opt].call(WpsLazyLoad, args);
					break;
				}	
			});	
		};	
		
	}($("#body"), $)//第一个参数是滚动条容器默认可以是window
